...
此对话脚本待真实执行；本地 synthetic 测试不能填为豆包通过。

1. 在原智能体更新完整技能；首次提示持久化安装。记录当前宿主证实的目录保留范围、实际Python/Skill/作品路径，doctor --repair输出。如果仅有/runtime，就明确告知无法确认跨重置保存。
2. 上传真实猫照片，不带指令。应先在5–15秒目标内给一句建议与选择，不调用图像处理工具。
3. 用户“去背景，给猫加帽子，保留绿色眼睛。”合并为一次宿主图片编辑，记录工具开始/结束和调用ID；不能自主第二次生成。候选可用立刻交付并停下。
4. 用户确认素材图。pixel --current，78×78、最多18色，立刻交付真实preview，依据本地analysis报告小色块建议。不再视觉长分析。
5. 用户“轻度去杂色。”route直接本地执行；记录修改格数、区域/色数前后与保护情况，新revision需重新确认，旧版仍在。
6. 用户“还是上一版。”恢复真实父版本并交付。用户“颜色减到12色。”只重映射当前网格，不调用AI。保护与12色冲突时解释真实原因，不能偷偷删保护色。
7. 用户确认最终像素效果，再生成正式PNG并单独展示。记录真实pattern展示回执。
8. 用户“导出PDF。”同一次route确认PNG并导出，不要求固定口令，不生成CSV。用户“PDF字大一点。”只换PDF布局，检查网格哈希不变。另说“下载清单”时才生成CSV。
9. 用户问“PDF是什么？”只解释；“眼睛改绿以后再PDF”先改图，不输出旧版本。四选项中强力简化必须明确目标色数，程序不猜。
10. 宿主确实支持模拟runtime重置时，在保留备份后测试：运行目录清空，实际持久Skill、venv、cache、projects仍在；doctor --repair只恢复链接，不clone/装全套依赖/全套测试。若无法模拟，标记该项待验证。

计时分别记录：首次建议（AI思考）、图片工具、每个本地命令、状态处理、预览交付、PDF绘制及脚本已返回到实际交付之间的等待。每项记录起止时间、证据消息/工具ID；未知就填null。超过60秒列出具体发生在工具、网络/安装、脚本或交付哪个环节，不以主观感觉代替。

真实观察记录模板：`{scope: doubao, phase, started_at, ended_at, evidence, outcome}`。通过record-stage和record-delivery写入当前项目events.jsonl；不要把文档里的占位消息ID当成真实记录。
