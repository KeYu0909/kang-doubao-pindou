...
新版优先使用[快速命令](fast-path.md)。route增加denoise/recolor/keep；pixel直接返回本地统计，数字去杂色菜单最多4项，其余通常2–3项。支持所有主要选择命令的--current，内部校验不代替真实用户展示/确认。仅减色重映射当前网格，旧版文档中的“从已保存原图重新配色”只适用于改变尺寸/输入/色板等重新采样场景。
