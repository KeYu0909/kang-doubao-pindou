...
ACTIONS={'confirm','pixel','png','export_pdf','export_csv','adjust','rollback'}
...
def present(a):
    with locked(a.project):
        s=load(a.project);r=current(s,a.revision);ctx=context(s,a.conversation)
        ...
        ctx['menu']=None;save(a.project,s)
    return {'presented':rec,'focus':ctx['focus']}

def offer(a):
    options=read_json(a.options)
    require(isinstance(options,list) and 1<=len(options)<=3,'OPTIONS: one to three choices')
    ...
    with locked(a.project):
        s=load(a.project);ctx=context(s,a.conversation);r=current(s,a.revision)
...
def classify(message):
    ...
    if re.search(r'大字|标准清晰|标准版|页数少|减少页数|字号.*大',t):
        return {'action':'export_pdf','args':{'layout':'large' if re.search(r'大字|字号.*大',t) else 'standard'}}
    ...
    if re.search(r'用量清单|csv',t) and re.search(r'下载|导出|给我',t):return {'action':'export_csv'}
...
def choose(message,ctx,s):
    ...
    choice_index={'1':0,'第一个':0,'第一项':0,'2':1,'第二个':1,'第二项':1,'3':2,'第三个':2,'第三项':2}.get(t)
...
def inspect(a):
    ...
    counts=w.grid_check(g);rare=[{'code':code,'count':n} for code,n in counts.items() if n<=3]
    hint=f'有{len(rare)}个色号各不超过3颗；小色块也可能是眼睛、高光或配饰，先查看再决定是否合并。' if rare else None
    return {'revision_id':r['revision_id'],'width':g['width'],'height':g['height'],'used_colors':len(counts),'color_limit':g['max_colors'],'nonempty_cells':sum(counts.values()),'rare_colors':rare,'hint':hint}
